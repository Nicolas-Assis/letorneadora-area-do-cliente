...
import { ProductDto } from 'src/api/dto/product/product.dto';
import { CategoryDto } from 'src/api/dto/category/category.dto';
...
//    dto.minStock = product.minStock;
    ...
    if (product.productCategories && product.productCategories.length > 0) {
      dto.categoryIds = product.productCategories.map((pc) => pc.category.id);
    }

    // Mapear imagens se existirem
    // if (product.productImages && product.productImages.length > 0) {
    //   dto.images = product.productImages.map(
    //     (pi) =>
    //       ({
    //         id: pi.id,
