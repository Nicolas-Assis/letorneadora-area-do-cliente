...
export class CategoriesService {
  ...
  async findAll(filter: FilterCategoriesDto): Promise<PaginatedResponseDto<CategoryDto>> {
    const { 
      page = 1, 
      limit = 10, 
      search, 
    ...
    const categoryDtos = this.categoryFactory.toDTOList(categories, includeChildren || includeParent || includeProductCount);

    return new PaginatedResponseDto(true, 'Categorias recuperadas com sucesso', categoryDtos, page, limit, total);
  }
    ...
    if (dto.parentId !== undefined) {
      ...
      if (dto.parentId) {
        ...
        if (await this.wouldCreateLoop(id, dto.parentId)) {
          throw new BadRequestException('Esta operação criaria um loop na hierarquia de categorias');
        }
  ...
  async remove(id: number): Promise<void> {
    ...
    if (category.productCategories && category.productCategories.length > 0) {
      throw new BadRequestException('Não é possível remover uma categoria que possui produtos associados');
    }
