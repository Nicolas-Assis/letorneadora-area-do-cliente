...
export class InventoryService {
  ...
  async findAll(filter: FilterInventoryDto): Promise<PaginatedResponseDto<InventoryDto>> {
    ...
    const inventoryDtos = this.inventoryFactory.toDTOList(inventories, includeProduct || includeWarehouse);
  ...
  async update(productId: number, warehouseId: number, dto: UpdateInventoryDto): Promise<InventoryDto> {
    const inventory = await this.inventoryRepository.findOne({
      where: { productId, warehouseId },
    });
  ...
  async adjustQuantity(productId: number, warehouseId: number, adjustment: number): Promise<InventoryDto> {
    const inventory = await this.inventoryRepository.findOne({
      where: { productId, warehouseId },
    });
