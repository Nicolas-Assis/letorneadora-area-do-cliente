...
export class OrdersService {
  ...
  private orders: Order[] = [
    new Order({
      ...
      description: 'Preciso de peças usinadas para projeto automotivo. Tolerância crítica de ±0.05mm.',
      desiredDeadline: 15,
    ...
    new Order({
      ...
      items: [], // Será preenchido no método de inicialização
      description: 'Eixos torneados para linha de produção. Especificação conforme desenho técnico anexo.',
      desiredDeadline: 10,
    ...
    new Order({
      ...
      items: [], // Será preenchido no método de inicialização
      description: 'Flanges em aço inox para sistema de tubulação industrial. Pressão de trabalho 16 bar.',
      desiredDeadline: 25,
    ...
    new Order({
      ...
      items: [], // Será preenchido no método de inicialização
      description: 'Componentes para drone de inspeção. Peso crítico, material alumínio aeronáutico.',
      desiredDeadline: 20,
  ...
  async findAll(queryDto: QueryOrdersDto): Promise<PaginatedResponseDto<Order>> {
    ...
    if (queryDto.urgent) {
      filteredOrders = filteredOrders.filter(o => 
        o.desiredDeadline && o.desiredDeadline <= 7
      );
    }
  ...
  async getOrderStats() {
    ...
    const avgOrderValue = this.orders
      .filter(o => o.totalAmount)
      .reduce((sum, o) => sum + o.totalAmount!, 0) / this.orders.filter(o => o.totalAmount).length;

    const avgDeliveryTime = this.orders
      .filter(o => o.estimatedDeadline)
      .reduce((sum, o) => sum + o.estimatedDeadline!, 0) / this.orders.filter(o => o.estimatedDeadline).length;

    const urgentOrders = this.orders.filter(o => 
      o.desiredDeadline && o.desiredDeadline <= 7 && 
      [OrderStatus.PENDING, OrderStatus.ANALYZING, OrderStatus.QUOTED, OrderStatus.APPROVED, OrderStatus.IN_PRODUCTION].includes(o.status)
    ).length;
